...
/**
 * Placeholder file system for future Specter-specific container handling.
 */
@FileSystemInfo(type = "specterfs",
		description = "Placeholder Specter file system", factory = SpecterFileSystem.MyFileSystemFactory.class)
public class SpecterFileSystem implements GFileSystem {
	 ...
	 */
	public SpecterFileSystem(FSRLRoot fsFSRL, ByteProvider provider) {
		this.fsFSRL = fsFSRL;
	...
	public void mount(TaskMonitor monitor) {
		monitor.setMessage("Opening " + SpecterFileSystem.class.getSimpleName() + "...");
	...
	public static class MyFileSystemFactory
			implements GFileSystemFactoryByteProvider<SpecterFileSystem>,
			GFileSystemProbeByteProvider {

		@Override
		public SpecterFileSystem create(FSRLRoot targetFSRL,
				ByteProvider byteProvider, FileSystemService fsService, TaskMonitor monitor)
				throws IOException, CancelledException {

			SpecterFileSystem fs = new SpecterFileSystem(targetFSRL, byteProvider);
			fs.mount(monitor);
			return fs;
		}
